...
import { Search, MapPin, BriefcaseBusiness, Users, TrendingUp, Star, ChevronRight, Play, Building, DollarSign, Mail, Filter } from 'lucide-react';
...
const HomePage = () => {
  ...
  useEffect(() => {
    if (jobPosts && jobPosts.length > 0) {
      const filtered = jobPosts.filter((job) => {
        // Search term filter (title, company, description, skills)
  ...
  }, [jobPosts, searchTerm, location, selectedCategory, selectedJobType]);

  const handleSearch = () => {
    // Search automatically happens through useEffect
    // This function can be used for analytics or additional actions
    console.log('Searching with:', { searchTerm, location, selectedCategory, selectedJobType });
  };
  ...
  const isFilterActive = searchTerm || location || selectedCategory || selectedJobType;
  
  // Display filtered jobs if filter is active, otherwise show all jobs
  const displayJobs = isFilterActive ? filteredJobs : (jobPosts || []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
        ...
        <div className="max-w-7xl mx-auto text-center mt-5">
          ...
          <div className="max-w-4xl mx-auto mb-12">
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none appearance-none bg-white"
                  >
                ...
                <div className="relative">
                  <BriefcaseBusiness className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 w-5 h-5" />
                  <select
                ...
                <button 
                  ...
                  className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-3 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 font-semibold shadow-lg"
                >
                  Clear Filters
                </button>
              </div>
              ...
              {(searchTerm || location || selectedCategory || selectedJobType) && (
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-slate-800 mb-4">
              {isFilterActive ? 'Search Results' : 'Featured Jobs'}
            </h2>
            <p className="text-slate-600 text-lg">
              {displayJobs?.length || 0} {displayJobs?.length === 1 ? 'job' : 'jobs'} found
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            {categories.map((category, index) => (
              <div 
                ...
                className="bg-white rounded-xl p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-slate-100 group cursor-pointer"
              >
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-blue-600 to-purple-600">
        <div className="max-w-4xl mx-auto text-center text-white">
          ...
          <a href="/register">
            <button className="bg-white text-blue-600 px-8 py-3 rounded-xl hover:bg-slate-100 transition-colors font-semibold cursor-pointer">
              Create Account
            </button>
            </a>
          </div>
